use colored::*;
use v8::{HandleScope, Value, Local, Function, Symbol};
...
pub fn log(scope: &mut HandleScope, _args : v8::FunctionCallbackArguments, _rv : v8::ReturnValue) -> () {
    let mut out : Vec<String> = vec![];
    for i in 0.._args.length() {
        out.push(inspect(scope, _args.get(i), Some(0)));
    }

    println!("{}", out.join(" "));
}

// Used for logging useful 
fn inspect(scope : &mut HandleScope, value : Local<Value>, level : Option<u8>) -> String {
    let lvl = level.unwrap_or(0);
    match type_of(value) {
        "array" => Colors::BracketMatch(lvl, format!("[ {} ]", inspect_array(scope, value, lvl))).to_string(),
        "function" => Colors::Special(inspect_function(scope, value)).to_string(),
        "string" => inspect_string(scope, value.into()),
        "number" => Colors::Number(format!("{}", value.to_rust_string_lossy(scope))).to_string(),
        "symbol" => Colors::Symbol(inspect_symbol(scope, value)).to_string(),
        "object" => Colors::BracketMatch(lvl, format!("{{\n{1}\n{0}}}", str::repeat("   ", lvl as usize), inspect_object(scope, value, lvl))).to_string(),
...
fn inspect_array(scope : &mut HandleScope, arr: Local<Value>, lvl : u8) -> String {
    ...
    for i in 0..(if array.length() > 32 {32} else {array.length()}) {
        ...
        items.push(inspect(scope, el, Some(lvl + 1)));
    }
...
fn inspect_object(scope : &mut HandleScope, object : Local<Value>, lvl: u8) -> String {
    ...
    for i in 0..props.length() {
        ...
        let name = inspect(scope, prop, Some(lvl+1));
        let value = obj.get(scope, prop.into()).unwrap();
        let val = inspect(scope, value, Some(lvl+1));
...
fn inspect_symbol(scope: &mut HandleScope, symbol: Local<Value>) -> String {
    ...
    return match sym.description(scope).is_undefined() {
        ...
        false => String::from(format!("Symbol({})", inspect(scope, d, Some(0))))
    };
}